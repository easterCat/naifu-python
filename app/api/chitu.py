...
def handle_chitu():
    ...
    for row in sheet.values:
        ...
        tem_instance = TemplateChitu(
            name=str(data['name']).strip(), author=str(data['author']).strip(), preview=str(data['preview']).strip(),
            prompt=str(data['prompt'] or '').strip(), prompt_zh=str(data['prompt_zh'] or '').strip(),
            n_prompt=str(data['n_prompt']).strip(),
            desc=str(data['desc'] or '').strip(),
            model=str(data['model']).strip(), file1=str(data['file1']).strip(), file2=str(data['file2']).strip(),
            file3=str(data['file3']).strip(), key_word=str(data['key_word']).strip(),
...
@api.route("/chi_tu/update")
def handle_chitu_update():
    total = 0
    wb = load_workbook('app/static/xlsx/NovelAI_cankaotu.xlsx')
    sheet = wb['工作表 1']
    for row in sheet.values:
...
@api.route('/chi_tu/download_img')
def chi_tu_download_img():
    total = 0
...
@api.route("/chi_tu/replace_url")
def chi_tu_replace_url():
    total = 0
    templates = TemplateChitu.query.all()
    ...
    for tem in templates:
        ...
        db.session.query(TemplateChitu).filter_by(id=tem_json['id']).update(new_data)
        db.session.commit()
        total += 1
        print(total)
...
def download_images(img_name, img_url, headers, proxy):
    r = requests.get(img_url, headers=headers, proxies={"http": "http://{}".format(proxy)})
...
def add(insert_data):
    me = TemplateChitu(name=insert_data['name'] or '暂无', prompt=str(insert_data['prompt']),
                       preview=insert_data['preview'],
                       author=insert_data['author'],
                       prompt_zh=insert_data['prompt_zh'], n_prompt=insert_data['n_prompt'], desc=insert_data['desc'],
                       model=insert_data['model'],
                       key_word=insert_data['key_word'],
                       key_word2=insert_data['key_word2'],
                       file1=insert_data['file1'], file2=insert_data['file2'], file3=insert_data['file3'])
    db.session.add(me)
    db.session.commit()
